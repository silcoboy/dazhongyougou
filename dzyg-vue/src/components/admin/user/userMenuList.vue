<template>
  <el-container>
    ...
    <el-header>
      <el-input v-model="queryInfo.userName"   placeholder="搜索用户"
                prefix-icon="el-icon-search"></el-input> 
		 	<el-input v-model="queryInfo.title"   placeholder="搜索标题"
		 	          prefix-icon="el-icon-search"></el-input> 			
...
<script>
  export default {
    ...
    methods: {
      //获取用户信息
      getDataInfo () {
        this.$http.get(this.API.getUserMenu, { params: this.queryInfo }).then((resp) => {
			   ...
			   this.$confirm('你正在操作删除，是否继续?', 'Tips', {
			   			   confirmButtonText: '确定',
			   			     ...
			   			     type: 'warning'
			   				   ...
			   				   this.$http.get(this.API.deleteMenu, { params: { 'id': row.id,status:0 } }).then((resp) => {
			   				     if (resp.data.code === 200) {
			   				       //删除成功后,回调更新数据
